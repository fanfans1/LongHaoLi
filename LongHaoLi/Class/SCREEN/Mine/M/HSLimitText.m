...
//    textField.font = [UIFont systemFontOfSize:13.0*SCREEN_WIDTH/414];
    [self  addSubview:textField];
    [textField becomeFirstResponder];
    self.textField = textField;
    
    UIView  *leftView  = [[UIView alloc] initWithFrame:RECTMACK(0, 0, 10, self.frame.size.height)];
...
- (void)addLabPlaceholder
{
    if(!self.labPlaceHolder){
        ...
        self.labPlaceHolder.font = [UIFont systemFontOfSize:13.0*SCREEN_WIDTH/414];
...
- (void)layoutSubviews
{
    ...
    if(self.labPlaceHolder){
       self.labPlaceHolder.frame = RECTMACK(TextViewPlaceHolderOrignX, TextViewPlaceHolderOrignY, TextViewPlaceHolderWidth, TextViewPlaceHolderHeight);
    }
}
- (void)setuptTextView
{
